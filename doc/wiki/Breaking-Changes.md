这些变动列出了不同版本间实现的差异，还有从何时起编译器被精简了及矛盾被修正了。

> 对于compiler/services API的重大更改，请参照[[API Breaking Changes]]页面。

# TypeScript 1.8

变更改变的完整列表请查看[breaking change issues](https://github.com/Microsoft/TypeScript/issues?q=is%3Aissue+milestone%3A%22TypeScript+1.8%22+label%3A%22Breaking+Change%22+is%3Aclosed)。

#### 现在在生成模块代码时开头会带有`"use strict";`

在ES6中，模块总是会在严格模式下被解析，但是对于非ES6程序而言，这不会在生成的代码中体现出来。从TypeScript 1.8开始，生成的模块代码始终是严格模式。这不应该会给你的代码带来任意可见的变化，因为TS把大多数严格模式下的错误当作错误并在编译阶段抛出来，但这也意味着之前在运行时才默默失败的TS代码现在会直接报错。你可以参考[MDN Article](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode)严格模式与非严格模式的详细对照列表。

若要禁用这个行为，传递`--noImplicitUseStrict`命令行参数或在`tsconfig.json`文件里进行设置。

#### 从模块中导出非本地名字

根据ES6/ES2015规范，从模块中导出非本地名字是个错误。
